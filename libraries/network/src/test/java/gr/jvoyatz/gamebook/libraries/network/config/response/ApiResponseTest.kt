package gr.jvoyatz.gamebook.libraries.network.config.response

import com.google.common.truth.Truth
import gr.jvoyatz.gamebook.libraries.network.response.ApiResponse
import gr.jvoyatz.gamebook.libraries.network.response.ApiResponse.ApiSuccess
import gr.jvoyatz.gamebook.libraries.network.response.ApiResponse.HttpError
import gr.jvoyatz.gamebook.libraries.network.response.ApiResponse.NetworkError
import gr.jvoyatz.gamebook.libraries.network.response.asError
import gr.jvoyatz.gamebook.libraries.network.response.asHttpError
import gr.jvoyatz.gamebook.libraries.network.response.asNetworkError
import gr.jvoyatz.gamebook.libraries.network.response.asSuccess
import gr.jvoyatz.gamebook.libraries.network.response.asUnexpectedError
import gr.jvoyatz.gamebook.libraries.network.response.getOrNull
...
import gr.jvoyatz.gamebook.libraries.network.response.isSuccessEmpty
import gr.jvoyatz.gamebook.libraries.network.response.onError
import gr.jvoyatz.gamebook.libraries.network.response.onHttpError
import gr.jvoyatz.gamebook.libraries.network.response.onNetworkError
import gr.jvoyatz.gamebook.libraries.network.response.onSuccess
import gr.jvoyatz.gamebook.libraries.network.response.onSuspendedError
import gr.jvoyatz.gamebook.libraries.network.response.onSuspendedHttpError
import gr.jvoyatz.gamebook.libraries.network.response.onSuspendedNetworkError
import gr.jvoyatz.gamebook.libraries.network.response.onSuspendedSuccess
import gr.jvoyatz.gamebook.libraries.network.response.onSuspendedUnexpectedError
import gr.jvoyatz.gamebook.libraries.network.response.onUnexpectedError
import gr.jvoyatz.gamebook.libraries.network.response.toFlow
import gr.jvoyatz.gamebook.libraries.network.response.toSuspendFlow
...
class ApiResponseTest {
    ...
    private val unknownError = ApiResponse.unexpectedError<String, String>(NullPointerException("unknownerror"))
    private val networkError = ApiResponse.networkError<String, String>(IllegalStateException("networkerror"))
    ...
    fun `test that unknownError() returns unknownError instance`(){
        ...
        //then
        Truth.assertThat(unknownError).isInstanceOf(ApiResponse.UnexpectedError::class.java)
    }
    ...
    fun `onUnknownError is executed when apiResponse is an UnknownError instance`(){
        ...
        //when
        unknownError.onUnexpectedError {
            isExecuted = true
        }
    ...
    fun `onSuspendedNetworkError is executed when apiResponse is an UnknownError instance`()= runTest{
        ...
        //when
        unknownError.onSuspendedUnexpectedError {
            isExecuted = true
        }
